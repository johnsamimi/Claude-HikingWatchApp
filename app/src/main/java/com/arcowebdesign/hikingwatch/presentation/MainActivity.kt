    ...
    when {
        ...
        trackingState is TrackingState.Completed -> {
            ...
            SummaryScreen(
                sessionId = sessionId,
                viewModel = viewModel,
                onDone = { /* resets to Idle via viewModel */ }
            )
        }

        // Ambient mode - minimal display
        isAmbient -> {
        ...
        // Main pager: Map (page 0) ↔ Stats (page 1)
            ...
            HorizontalPager(state = pagerState) { page ->
                when (page) {
                    0 -> MapScreen(viewModel = viewModel, modifier = Modifier.fillMaxSize())
                    1 -> StatsScreen(viewModel = viewModel, modifier = Modifier.fillMaxSize())
                }
            }
        }
    }
...
private fun AmbientScreen(viewModel: HikingViewModel) {
    ...
    // Dark minimal screen for ambient mode
...
val Modifier.fillMaxSize get() = this.then(Modifier.fillMaxSize())
