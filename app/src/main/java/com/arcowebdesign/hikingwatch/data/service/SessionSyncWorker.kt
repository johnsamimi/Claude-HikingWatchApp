...
class SessionSyncWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        return try {
            // Placeholder: sync completed sessions to companion app or cloud
            // Gracefully succeeds even without a connection
            Result.success()
        } catch (e: Exception) {
            Result.retry()
        }
    ...
    companion object {
        ...
        fun schedule(context: Context) {
            val request = PeriodicWorkRequestBuilder<SessionSyncWorker>(30, TimeUnit.MINUTES)
                .setConstraints(
                    Constraints.Builder()
